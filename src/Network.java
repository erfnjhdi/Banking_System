...
public class Network extends Thread {
     ...
     */
     Network( )
      { 
         ...
         spaces1 = new Semaphore(maxNbPackets);
     ...
     */
        ...
        {
        			try {
                  	  spaces1.acquire(); //wait until there is space in inComingPacket
                	  mutex1.acquire();  //lock the access to inComingPacket
        		  ...
        		  mutex1.release();          //release lock on inComingPacket
        		  items1.release();          //signal an item is available in inComingPacket
     ...
     */
        ...
        {
     			try {
	           	     items2.acquire();          //wait for item in outGoingPacket
	           	     mutex2.acquire();          //lock access to outGoingPacket
      			}catch (InterruptedException e) {
      	            e.printStackTrace();
      	        }
        		 ...
        		 mutex2.release();          //release lock on outGoingPacket
        		 spaces2.release();         //signal space is available in outGoingPacket
     ...
     */
        ...
        {
     			try {
            	    spaces2.acquire();         //wait for space in outGoingPacket buffer
            	    mutex2.acquire();          //lock access to outGoingPacket with mutex
     			}catch (InterruptedException e) {
     	            e.printStackTrace();
     	        }
        	    ...
        	    mutex2.release();          //release the lock on outGoingPacket
        	    items2.release();          //signal that an item is available in outGoingPacket
     ...
     */
       public static boolean transferIn(Transactions inPacket) throws InterruptedException
        {
      			try {
	       	    	 items1.acquire();          //wait for item in inComingPacket
	       	    	 mutex1.acquire();          //lock access to inComingPacket
     			}catch (InterruptedException e) {
     	            e.printStackTrace();
     			}
    		     ...
    		     mutex1.release();          //release lock on inComingPacket
    		     spaces1.release();         //signal space is available in inComingPacket
    		     return true;
        }   
